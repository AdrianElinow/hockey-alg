...
def contract( ranges, player, goalie=False, debug=False):

    if not (player.isGoalie == goalie):
        raise ValueError()
    ...
    for attribute in ranges:
        ...
        for rng, val in ranges[attribute].items():
            ...
            if low <= player_attributes[attribute] <= high:
                values.append(val)
    """

    return average

def performance_bonus( factors, player, goalie=False, debug=False ):

    # assert goalie or not
    if not (goalie == player.isGoalie):
        raise ValueError()      
...
def conduct_testing( player_attr_brackets, goalie_attr_brackets, player_performance_factors, goalie_performance_factors ):
    ...
    for name, player in players.items():
        ...
        if player.isGoalie:
            ...
            perf_bonus  =   performance_bonus(goalie_performance_factors, player )
        ...
        if player.position == "defense":
            ...
            p_escalator = escalator(player_escalator, player, True)
